...
def _build_route_specific_rules(evidence_payload: Any) -> str:
    ...
    common_rules = (
        ...
        "10) Never omit the date in barcode log analysis answers.\n"
        "11) If evidence contains notionPlaybook/notion references, include a '참고 플레이북' section and cite only those references."
    )
    ...
    return (
        common_rules
        + "\n"
        "12) For error-focused analysis, add these sections in order:\n"
        "    - 에러 요약\n"
        "    - 관찰된 에러 패턴(시간/컴포넌트/핵심 메시지)\n"
        "    - 가능 원인(근거 라인 기반, 확실/추정 구분)\n"
        "    - 즉시 확인할 항목(로그/메트릭/설정)\n"
        "    - 우선 조치(1~3순위)\n"
        "13) For causes, never guess without evidence. If inferred, prefix with '추정:'."
    )
